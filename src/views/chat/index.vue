...
async function onConversation() {
  ...
  if (usingContext.value) {
    // 获取历史消息记录
    const contextMessages = dataSources.value
      .slice(-6) // 获取最近的6条消息，可以根据需要调整数量
      .filter(item => !item.loading && !item.error) // 过滤掉加载中和错误的消息
      .map(item => ({
            ...
            if (currentText) {
              updateChat(
                ...
                dataSources.value.length - 1,
                {
                  ...
                  error: false,
                  loading: true,
                  conversationOptions: options, // 保存当前的上下文选项
                  requestOptions: { prompt: message, options: { ...options } },
                },
              )
